...
def main():
    ...
    dataset_train = Face(mode='train', filename=filename, resize=224)
    dataset_val = Face(mode='val', filename=filename, resize=224)
    ...
    weight_path = "weight"
    if opt.checkpoint != 0:
        model.load_state_dict(torch.load(os.path.join(weight_path, "tsm_model_%s_%d.pth" %(filename, opt.checkpoint))))
    ...
    for epoch in range(opt.checkpoint, opt.epochs):
        ...
        if epoch % 1 == 0:
            ...
            torch.save(model.state_dict(), os.path.join(weight_path, "tsm_model_%s_%d.pth" %(filename, epoch+1)))
            with open("log/tsm_%s.txt" %(filename), "a+") as f:

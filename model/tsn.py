...
class TSN(nn.Module):
    ...
    def forward(self, input, no_reshape=False):
        ...
        if self.reshape:
            ...
            # print('out:', output.shape, output.squeeze(1).shape)
...
if __name__ == '__main__':
    ...
    model_rgb = TSN(num_class=2, num_segments=3, new_length=1, modality='RGB',
                    base_model='BNInception',
                    dropout=0.8, partial_bn=True)
    x = torch.rand(4*3, 3, 224, 224)
    print(model_rgb(x).shape)
    # summary(model_rgb, (3, 3, 224, 224), device='cpu')
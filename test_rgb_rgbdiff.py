...
# python test_rgb_rgbdiff.py --data NeuralTextures --compression c40 --mode C -c_rgb 8 -c_diff 7
...
def evaluate(model_rgb, model_diff, loader_rgb, loader_diff):
    model_rgb.eval()
    model_diff.eval()
    ...
    return correct_rgb / total, correct_diff / total, correct_avg / total
...
def main():
    ...
    if opt.data == "All":
        ...
        rgb_acc, diff_acc, avg_acc = evaluate(model_rgb, model_diff, loader_val_rgb, loader_val_diff)
        print("model_%s_%d：   rgb_acc：%.6f  diff_acc：%.6f  avg_acc：%.6f" % (filename, opt.checkpoint_rgb, rgb_acc, diff_acc, avg_acc))

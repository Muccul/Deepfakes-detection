...
#  python train_xception_notrans.py --data Deepfakes --compression c40 --mode C --checkpoint 0
...
def main():
    ...
    weight_path = "weight"
    if opt.checkpoint != 0:
        model.load_state_dict(torch.load(os.path.join(weight_path, "model_notrans_%s_%d.pth" %(filename, opt.checkpoint))))
    ...
    for epoch in range(opt.checkpoint, opt.epochs):
        ...
        if epoch % 1 == 0:
            ...
            torch.save(model.state_dict(), os.path.join(weight_path, "model_notrans_%s_%d.pth" %(filename, epoch+1)))
            with open("log/notrans_%s.txt" %(filename), "a+") as f:

...
# python test_rgb_flow.py --data NeuralTextures --compression c40 --mode C -c_rgb 8  -c_flow 12
...
def evaluate(model_rgb, model_flow, loader_rgb, loader_flow):
    model_rgb.eval()
    model_flow.eval()
    ...
    for (x_rgb, y_rgb),  (x_flow, y_flow) in tqdm(zip(loader_rgb, loader_flow)):
        x_rgb, x_flow, y_rgb = x_rgb.to(device), x_flow.to(device), y_rgb.to(device)
        ...
        with torch.no_grad():
            ...
            logits_avg = (logits_rgb + logits_flow) / 2
    ...
    return correct_rgb / total, correct_flow / total, correct_avg / total


def evaluate_all(model_rgb, model_flow, loader_rgb, loader_flow):
    model_rgb.eval()
    model_flow.eval()
...
def main():
    ...
    if opt.data == "All":
        TP, TN, FP, FN, correct_0, correct_1, correct_2, correct_3, correct_4, total_0, total_1, total_2, total_3, total_4 = evaluate_all(
            model_rgb, model_flow, loader_val_rgb, loader_val_flow)
        ...
        rgb_acc, flow_acc, avg_acc = evaluate(model_rgb, model_flow, loader_val_rgb, loader_val_flow)
        print("model_%s_%d：   rgb_acc：%.6f   flow_acc：%.6f  avg_acc：%.6f" % (
        filename, opt.checkpoint_rgb, rgb_acc, flow_acc, avg_acc))
